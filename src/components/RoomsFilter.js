...
import { useContext } from 'react';//use the context
import {RoomContext} from '../Context';
import Title from '../components/Title';

//get all unique values...shall help us to get the uniqueas
...
export default function RoomsFilter({rooms}) {
    ...
    const {
        ...
        price,minPrice,maxPrice,minSize,maxSize,breakfast,pets
    } = context;
//get unique types
...
people = people.map((item,index) => {return <option key={index} value={item}>{item}</option>})
  return (
    <section className="filter-container">
      ...
      <input type="range" name="price" id="price" min={minPrice} max={maxPrice}
